...
export function ProductsPage() {
  ...
  useEffect(() => { setBackgroundReady(true); }, []);
  ...
  const scriptsController = useScriptsController({ initialFilters: { productId: productIdParam ?? null, status: [], isApproved: null }, enabled: backgroundReady });
  const imagesController = useImagesController({ initialFilters: { productId: productIdParam ?? null, status: [], imageType: null }, enabled: backgroundReady });
  ...
  // Loading state
  const isLoading = productsController.isLoading || campaignsController.isLoading;
